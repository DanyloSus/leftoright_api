...
async def test_update_tournament(auth_client):
    created = (
        await auth_client.post("/api/tournaments/", json={"name": "Old Name"})
    ).json()
    resp = await auth_client.patch(
        f"/api/tournaments/{created['id']}", json={"name": "New Name"}
    )
    assert resp.status_code == status.HTTP_200_OK
...
async def test_delete_tournament(auth_client):
    created = (
        await auth_client.post("/api/tournaments/", json={"name": "To Delete"})
    ).json()
...
async def test_other_user_cannot_update_tournament(client):
    ...
    resp = await client.patch(
        f"/api/tournaments/{created['id']}", json={"name": "Hijacked"}
    )
