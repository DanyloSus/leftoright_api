...
async def test_create_user(client):
    resp = await client.post(
        "/api/users/", json={"email": "alice@example.com", "username": "alice"}
    )
...
async def test_get_user_by_id(client):
    created = (
        await client.post(
            "/api/users/", json={"email": "c@example.com", "username": "c"}
        )
    ).json()
...
async def test_update_user(client):
    created = (
        await client.post(
            "/api/users/", json={"email": "d@example.com", "username": "d"}
        )
    ).json()
    resp = await client.patch(
        f"/api/users/{created['id']}", json={"username": "updated"}
    )
    assert resp.status_code == status.HTTP_200_OK
...
async def test_delete_user(client):
    created = (
        await client.post(
            "/api/users/", json={"email": "e@example.com", "username": "e"}
        )
    ).json()
